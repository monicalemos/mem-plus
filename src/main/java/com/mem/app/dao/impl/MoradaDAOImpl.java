...
public class MoradaDAOImpl implements MoradaDAO {
	...
	@Override
	public void saveOrUpdate(Morada morada) {
		if (morada.getIdMorada() > 0) {
			...
			// insert
			String sql = "INSERT INTO morada (pais, regiao, cidade, idMorada)"
					+ " VALUES (?, ?, ?, ?)";
			jdbcTemplate.update(sql, morada.getPais(), morada.getRegiao(), morada.getCidade(), morada.getIdMorada() );
		}
	}
	...
	public Morada get(int moradaId) {
		...
		return jdbcTemplate.query(sql, new ResultSetExtractor<Morada>() {
			...
			@Override
			public Morada extractData(ResultSet rs) throws SQLException,
			DataAccessException {
				if (rs.next()) {
